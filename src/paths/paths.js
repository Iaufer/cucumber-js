...
async function expandPaths(cwd, unexpandedPaths, defaultExtension) {
    const expandedPaths = await Promise.all(unexpandedPaths.map(async (unexpandedPath) => {
         ...
         */
        if (node_path_1.default.extname(unexpandedPath) !== '') {
            const candidate = node_path_1.default.isAbsolute(unexpandedPath)
                ? unexpandedPath
                : node_path_1.default.join(cwd, unexpandedPath);
            try {
                if ((await fs_1.default.stat(candidate)).isFile()) {
                    return [candidate];
                }
            }
            catch {
